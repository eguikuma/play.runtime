...
### <strong>3. read.user-space / read.kernel-space との関係</strong>
...
[ アプリケーション ]
        ↑
[ ランタイム / 言語実行基盤 ]      ← read.runtime（ここ）
        ↑
[ OS（ユーザー空間） ]              ← read.user-space
        ↑
[ Linux カーネル（カーネル空間） ]   ← read.kernel-space
```

read.user-space では「OS がプログラムをどう動かすか」を学びました

read.kernel-space では「OS 自身がどう動いているか」を学びました
...
- <strong>read.user-space</strong>
  - プロセス、スレッド、メモリ構造の基礎
- <strong>read.kernel-space</strong>
